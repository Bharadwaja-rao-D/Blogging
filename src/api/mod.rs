...
use crate::db::content::{self, specific_content};
use crate::Pool;
...
pub async fn add_student(db_pool: web::Data<Pool>, path: web::Path<String>) -> std::io::Result<HttpResponse>{
    let db_pool = db_pool.get().unwrap();
    let new_student = path.into_inner();
